...
description: "Full API reference for WPIT's oblique EMIC–ion module: wpi_params, equations of motion, nonlinear trapping diagnostics, and wave number evolution for ion dynamics."
---
...
```python
dkpar_dt = emic.dkpardt(ppar, m, gamma, psi, dkdz)
```
...
<Note>
  There is no equivalent of `dkpardt` in `whistler_electron_mod`. The EMIC ion interaction requires explicit tracking of the parallel wave number evolution because the EMIC dispersion surface varies significantly with field-line position and ion composition.
</Note>
...
dkpar = emic.dkpardt(ppar, mi, gamma, eta, dkdz)
