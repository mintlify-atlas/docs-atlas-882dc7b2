...
description: "API reference for WPIT's RayUtils_mod: read and append ray files, compute resonant energies along ray paths, visualise Landau damping, and derive wave-power enhancement histograms."
---
...
Reads the raw ray file and the corresponding `<ray_file_name>_damping.csv` produced by `landau_damping`, then computes a comprehensive set of derived quantities at each time step and saves the result as `<ray_file_name>_appended.csv`. The raw ray data is truncated to steps where the normalised wave power remains above 1% (0.01).
...
<ResponseField name="(side-effect)" type="file">
  Writes `<ray_file_name>_appended.csv`. The file contains all time steps where `damp > 0.01` and includes the following columns beyond those in the raw ray:
