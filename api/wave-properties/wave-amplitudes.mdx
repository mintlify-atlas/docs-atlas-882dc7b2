...
<Note>
  In all component formulae, `f1 = P − η² sin²θ`. The polarisation ratios diverge
...
<Note>
  ...
  then passed to Bell (1984) relations to obtain the remaining components.
</Note>
...
<ResponseField name="Bwave" type="float">
  Wave magnetic field amplitude at latitude λ [T].
  Computed as `Bw0 * (tanh(dir * shape * rad2deg(λ) − 2 * location) + 1) / 2`
  where `dir` is +1 for north and −1 for south.
</ResponseField>
...
<ResponseField name="Bwave" type="float">
  ...
  is applied, both anchored at `location` degrees.
</ResponseField>
