...
$$
T_d = \frac{2\pi q B R_E^3}{m v^2} \cdot \frac{1}{L R_E} \cdot \left[1 - \frac{1}{3}\sin^{0.62}\!\alpha_{\text{eq}}\right]^{-1}
$$
...
<Note>
  The function internally uses `env.const.qe` (electron charge) for the charge factor regardless of the species passed. Verify source code behaviour when applying to ion drift calculations.
</Note>
