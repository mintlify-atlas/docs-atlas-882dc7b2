...
### `wpi.nonlinear_C1p(pper, ppar, w2, Exw, Eyw, m_res, wce, kz, gamma)` — Right-Hand Coupling C1⁺

Analogous to C1⁻ but for the right-hand polarization component EwL = (Ex − Ey)/2.
...
Ek = Ek_keV * 1e3 * env.const.qe   # J
alpha = env.aeq2alpha(aeq, L, lamda)
ppar, pper = env.initial_velocity(Ek, alpha, env.const.me)
