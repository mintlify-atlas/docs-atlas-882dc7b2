---
title: "Working with Ray-Tracing Output Files in WPIT"
sidebarTitle: "Ray Tracing"
description: "How to read, append, and analyse ray-tracing .ray files using WPIT's RayUtils_mod: resonance conditions, Landau damping, and wave amplitude enhancement factors."
---
...
`read_input_ray(ray_file_name)` reads a raw `.ray` file and returns a pandas `DataFrame` with all 36 columns named according to the convention above, plus a derived `freq` column (Hz).
...
# ['ray', 'ray_stop', 'time', 'posx', 'posy', 'posz',
